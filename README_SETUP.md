...
관리자 페이지(`admin.html`) 및 HR 관리 탭 접근용 비밀번호는 코드에 평문으로 저장되어 있으므로
**공개 저장소에는 절대 기재하지 마세요.**

- 운영 비밀번호 위치: `admin.js` 내 `ADMIN_PASSWORD`, `script.js` 내 `HR_PASSWORD`
- 변경 시 팀 내부 안전 채널(1Password 등)로만 공유
- 노출이 의심되면 즉시 변경 후 HR 데이터 재암호화 필요
